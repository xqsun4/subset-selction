...
"""A Unified Perspective on Regularization and Perturbation in Differentiable Subset Selection
    ...
    function: select k pixels from MNIST by Regularized Autoencoder
...
class RegularizedSelect(Layer):
    ...
    def __init__(self, output_dim, start_temp = 10.0, min_temp = 0.1, alpha = 0.9999, **kwargs):
        ...
        super(RegularizedSelect, self).__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        super(RegularizedSelect, self).build(input_shape)
    ...
    def call(self, X, training = None):
        ...
        samples = K.softmax(self.logits/temp) # prob of each pixel 
...
class StopperCallback(EarlyStopping):
    ...
    def on_epoch_begin(self, epoch, logs = None):
        print('mean max of probabilities:', self.get_monitor_value(logs), '- temperature', K.get_value(self.model.get_layer('regularized_select').temp))

        
    def get_monitor_value(self, logs):
        monitor_value = K.get_value(K.mean(K.max(K.softmax(self.model.get_layer('regularized_select').logits), axis = -1)))
...
if __name__ =="__main__":
    ...
    for i in range(10):
        k = 50
        a = RegularizedSelect(output_dim=k, start_temp = 10.0, min_temp = 0.1,  name = 'regularized_select')
        ...
        # print(model.summary())
        model.fit(x_train, x_train, batch_size = 234, epochs = epoch, callbacks = [stopper_callback])
        
        regularized_layer = model.get_layer(name = 'regularized_select')
        probabilities = K.get_value(K.softmax(regularized_layer.get_weights()[0]))
        indices = K.get_value(K.argmax(regularized_layer.get_weights()[0]))
        print('prob: ', probabilities)
        print('indices:', indices)
        
        expand = tf.one_hot(indices, 784)
        topk = tf.reduce_sum(expand, axis = 0).numpy()
        # topk = tf.reduce_sum(expand, axis = 0).eval()
        print(np.sum(topk))
        print('topk:', topk)
        ...
        path = 'table'
        text = 'regularized_kfold_mnist_selection_{}_{}.csv'.format(i, k)
