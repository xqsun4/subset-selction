'''         Balın et al. Concrete Autoencoders: Differentiable Feature Selection and Reconstruction
            ...
            function: select k pixels from MNIST datasets by using Concrete Autoencoder.
'''
...
def run_experiment(name, train, val, test, K, hidden_units, num_epochs, batch_size, learning_rate, dropout, start_temp = 10.0, min_temp = 0.01, tryout_limit = 5):
    ...
    eval_spec = tf.estimator.EvalSpec(input_fn = eval_input_fn, steps = None, throttle_secs = 0) #requires module: tensorflow_estimator
    ...
    for i in range(tryout_limit):
        ...
        # model_dir = ('concrete_model_dir_concrete')
        ...
        regressor = tf.estimator.Estimator(
            ...
            model_dir = model_dir,
            config = tf.estimator.RunConfig( save_checkpoints_steps = epochs_per_evaluation * steps_per_epoch, 
                                            save_summary_steps = steps_per_epoch, log_step_count_steps = steps_per_epoch, 
